...
function redirigirSiNoHayDatos() {
  ...
  const nombre = document.getElementById('nombre-asesorado').value
...
function buscarAsesoria() {
  ...
  if (nombre === '' || apellidoPaterno === '' || apellidoMaterno === '') {
    ...
    const url = `http://localhost:3000/asesorias/buscar?nombre=${encodeURIComponent(
      nombre
    )}&apellido_paterno=${encodeURIComponent(
      apellidoPaterno
    )}&apellido_materno=${encodeURIComponent(apellidoMaterno)}`
