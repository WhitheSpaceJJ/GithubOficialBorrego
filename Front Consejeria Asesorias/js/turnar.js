...
function registrarTurno() {
  ...
  if (hora.value === '' || minuto.value === '') {
    ...
    fetch(
      'http://localhost:3000/asesorias/' + asesoria.datos_asesoria.id_asesoria,
      options
    )
      .then(response => {
        if (!response.ok) {
      ...
      .then(data => {
        ...
        let mensaje =
          'El registro del turno de la asesoria se ha realizado correctamente.'
        mensajeModal.innerHTML = mensaje
      })
      .catch(error => {
        const miAlerta = document.getElementById('miAlerta')
...
function rellenarAsesoria(asesoria) {
  ...
  const numExterior =
    asesoria.persona.domicilio.numero_exterior_domicilio || '----------'
  const numInterior =
    asesoria.persona.domicilio.numero_interior_domicilio || '----------'
