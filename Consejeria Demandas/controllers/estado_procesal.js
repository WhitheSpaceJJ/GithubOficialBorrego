...
const obtenerEstadoProcesal = async (req, res) => {
    try {
        const { id } = req.params
        const estado_procesal = await estado_procesalDAO.obtenerEstadoProcesalPorId(Number(id))
...
const actualizarEstadoProcesal = async (req, res) => {
    try {
        const { id } = req.params
        const { descripcion_estado_procesal, fecha_estado_procesal, id_proceso_judicial } = req.body
        await estado_procesalDAO.actualizarEstadoProcesal(Number(id), {
            descripcion_estado_procesal,
            fecha_estado_procesal,
            id_proceso_judicial
        })
        const actualizado = await estado_procesalDAO.obtenerEstadoProcesalPorId(Number(id))
